...
public abstract class BaseRobot {
    ...
    boolean clearShot(MapLocation loc) {
    	RobotInfo[] friendlies = rc.senseNearbyRobots(-1, rc.getTeam());
		for(RobotInfo r: friendlies){
			if(Math.abs(rc.getLocation().directionTo(r.location).radians - rc.getLocation().directionTo(loc).radians) < .3){
				return false;
			}		
		}
		TreeInfo[] trees = rc.senseNearbyTrees();
		for(TreeInfo r: trees){
			if(Math.abs(rc.getLocation().directionTo(r.location).radians - rc.getLocation().directionTo(loc).radians) < .3 ){
				return false;
			}		
		}
