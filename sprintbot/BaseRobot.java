	...
	void dodgeBullets() throws GameActionException{
		...
		if(bullets.length > 0){
			for(BulletInfo b: bullets){ 
				...
				if(rc.getLocation().distanceTo(bulletTarget) < GameConstants.MAX_ROBOT_RADIUS){
					if(!rc.hasMoved() && rc.canMove(rc.getLocation().directionTo(bulletTarget).rotateLeftDegrees(90))){
